...
#The Hashes were incorrectly coded. Colon is used for symbols, but keys are strings.
...
def calculate_recipients
  ...
  SUBSCRIBERS.each do |subb|
    if !UNSUBSCRIBED.include?(subb)
      stillSubbed.push(subb)
    end
  end
...
def print_recipients
  ...
  recipients = calculate_recipients   #an array
  reciplist = ""                      #empty string

  for i in 0..(recipients.length-2)
    reciplist += recipients[i] +", "  #fill the string from the array, but last element doesn't need comma
  end
  reciplist += recipients[recipients.length-1]    #add last element since excluded from the loop

  puts reciplist   #print it
end
